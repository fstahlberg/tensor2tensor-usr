...
class TransformerExt(t2t_model.T2TModel):
  ...
  def decode(self,
    ...
    decoder_output = transformer_decoder(
        ...
        pop_decoder_bias=pop_decoder_bias,
        nonpadding=nonpadding,
        decoder_input_raw=decoder_input_raw,
  ...
  def model_fn_body(self, features):
    ...
    try:
      ...
      pass
    decoder_input, decoder_self_attention_bias, terminal_decoder_bias, nonterminal_decoder_bias, pop_decoder_bias, pos_signals = transformer_prepare_decoder(
    ...
    return self.decode(decoder_input, encoder_output,
                       ...
                       terminal_decoder_bias=terminal_decoder_bias, 
                       nonterminal_decoder_bias=nonterminal_decoder_bias,
                       decoder_input_raw=tf.squeeze(features["targets_raw"], axis=[2, 3]),
                       pop_decoder_bias=pop_decoder_bias,
                       pos_signals=pos_signals)
...
def transformer_prepare_decoder(targets, hparams, features=None):
  ...
  return (decoder_input, decoder_self_attention_bias, terminal_decoder_bias, nonterminal_decoder_bias, pop_decoder_bias, pos_signals)
        ...
        for layer_type in _iter_layer_types(hparams.decoder_layer_types, layer):
          if layer_type == "self_att":
            ...
            with tf.variable_scope("enc_pop_att"):
              enc_x = usr_utils.expand_memory_by_pop(pop_decoder_bias, encoder_output, offset=0)
            ...
            with tf.variable_scope("encdec_pop_pos_attention"):
              src_pos = tf.cumsum(tf.cast(pop_decoder_bias, tf.float32), axis=1)
              src_pos_embed = embed_position_signal_sine(
                  src_pos, hparams.hidden_size // 2, max_timescale=5.0e3)
              x_with_src_pos = tf.concat(
                  [common_layers.layer_preprocess(x, hparams), src_pos_embed], 2)
              x_with_src_pos.set_shape([None, None, hparams.hidden_size + hparams.hidden_size // 2])
...
def _iter_layer_types(layer_types_param, layer):
  for layer_type in layer_types_param.split(","):
    if ":" in layer_type:
      ...
      if len(parts) == 2:
        min_layer = 0
        max_layer = int(parts[1])
      else:
        min_layer = int(parts[1]) if parts[1] else 0
        max_layer = int(parts[2]) if parts[2] else 10000
      if layer < max_layer and layer >= min_layer:
        yield parts[0]
    else:
      yield layer_type
...
def _get_pop_bias(labels, hparams):
  try:
    squeezed_labels = tf.squeeze(labels, [2, 3])
    return tf.equal(squeezed_labels, hparams.pop_id)
  except AttributeError:
    pass
  return None
